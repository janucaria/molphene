...
namespace molphene {
    ...
    void color_light_buffer::reserve(GLuint n) {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(vec3f) * capacity_, nullptr, GL_STATIC_DRAW);
        
        glBindBuffer(GL_ARRAY_BUFFER, normal_buffer_);
        glBufferData(GL_ARRAY_BUFFER, sizeof(vec3f) * capacity_, nullptr, GL_STATIC_DRAW);
        
        glBindBuffer(GL_ARRAY_BUFFER, color_buffer_);
        glBufferData(GL_ARRAY_BUFFER, sizeof(colour) * capacity_ , nullptr, GL_DYNAMIC_DRAW);
    }
    
    void color_light_buffer::push(GLsizeiptr size, const vec3f * posdat, const vec3f * normdat, const colour * coldat) {
        if(!size) return;
    ...
    void color_light_buffer::set_position_data(GLintptr offset, GLsizeiptr size, const vec3f *data) {
        glBindBuffer(GL_ARRAY_BUFFER, position_buffer_);
        glBufferSubData(GL_ARRAY_BUFFER, offset * sizeof(vec3f), size * sizeof(vec3f), data);
    }
    
    void color_light_buffer::set_normal_data(GLintptr offset, GLsizeiptr size, const vec3f *data) {
        glBindBuffer(GL_ARRAY_BUFFER, normal_buffer_);
        glBufferSubData(GL_ARRAY_BUFFER, offset * sizeof(vec3f), size * sizeof(vec3f), data);
    }
    
    void color_light_buffer::set_color_data(GLintptr offset, GLsizeiptr size, const colour *data) {
        glBindBuffer(GL_ARRAY_BUFFER, color_buffer_);
