...
void
PDBParser::handleATOMRecord(molecule& mol)
{
...
void
PDBParser::handleMODELRecord(molecule& mol)
{
  currentModelPtr = &mol.addModel();
  currentChainPtr = nullptr;
}
...
void
PDBParser::handleCONECTRecord(molecule& mol)
{
...
void
PDBParser::handleTERRecord(molecule& mol)
{
  ...
  for(; compoundIt != compoundEndIt; ++compoundIt) {
    ...
    PDBParser::ResidueBondPairMap::const_iterator bpairsIt =
     resBondPairs.find(resName);
...
bool
PDBParser::buildBond(compound& comp1,
...
void
PDBParser::parse(molecule& mol, std::istream& stream)
{
  ...
  while(std::getline(stream, line_)) {
    ...
    } else if(recordName.compare("ATOM") == 0 ||
              recordName.compare("HETATM") == 0) {
...
std::string
PDBParser::column(unsigned int start, unsigned int end)
{
  return line_.substr(start - 1, end - start + 1);
}

float
PDBParser::getReal(unsigned int start, unsigned int end)
{
  return static_cast<float>(strtod(column(start, end).c_str(), nullptr));
}

unsigned int
PDBParser::getInteger(unsigned int start, unsigned int end)
{
  return static_cast<unsigned int>(
   strtol(column(start, end).c_str(), nullptr, 0));
}

char
PDBParser::getChar(unsigned int start)
{
 ...
 {{"ALA",
  ...
  {"GLY",
   PDBParser::BondPairList({{"N", "CA"},
                            ...
                            {"OXT", "HXT"}})},
  {"HIS",
   PDBParser::BondPairList({{"N", "CA"},    {"N", "H"},     {"N", "H2"},
                            {"CA", "C"},    {"CA", "CB"},   {"CA", "HA"},
                            {"C", "O"},     {"C", "OXT"},   {"CB", "CG"},
  ...
  {"LYS",
   PDBParser::BondPairList(
    {{"N", "CA"},   {"N", "H"},    {"N", "H2"},   {"CA", "C"},   {"CA", "CB"},
     {"CA", "HA"},  {"C", "O"},    {"C", "OXT"},  {"CB", "CG"},  {"CB", "HB2"},
     {"CB", "HB3"}, {"CG", "CD"},  {"CG", "HG2"}, {"CG", "HG3"}, {"CD", "CE"},
     {"CD", "HD2"}, {"CD", "HD3"}, {"CE", "NZ"},  {"CE", "HE2"}, {"CE", "HE3"},
